...
public class AlienBase : MonoBehaviour
{
	...
	public float UpdateGrowingPartsTime = 0.5f;
	public float UpdateGrowingPartsTimer = 0f;
	...
	void Update () {
		...
		UpdateGrowingPartsTimer += Time.deltaTime;
		...
		if (Application.isPlaying)
		{
			...
			if (UpdateBaseTimer >= UpdateBaseTime)
			{
				...
				StateOrder[currentState].Update(UpdateBaseTime);
	...
	{
		if (!updateScaleInstant && UpdateGrowingPartsTimer < UpdateGrowingPartsTime)
		{
			return;
		}
		UpdateGrowingPartsTimer = 0f;
